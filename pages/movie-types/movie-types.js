...
Page({
  ...
  getMovieListData(types,id){
    ...
    var total = this.data[types].total || 9999
    var url = app.globalData.ajaxBase 
    //if(offset >= total){
    //  return
    //}
    if (id>0){
      url += app.globalData.newslist + '?id=' + id + '&pageno=' + pageno
    }else if (types == 'newslist'){
      url += app.globalData.newslist + '?pageno=' + pageno
    } else if (types == 'banner'){
      url += app.globalData.banner +'?pageno=' + pageno
    }
    wx.showToast({
      title:'加载中',
      icon:'loading',
    ...
    wx.request({
      ...
      success:res => {
        ...
        this.setData({ [types]: { total, pageno,typename,id,movies}})
  ...
  getSearchData(kw) {
    ...
    var pageno = this.data[types].pageno || 1
    var total = this.data[types].total || 9999
    var url = app.globalData.ajaxBase + app.globalData.searchURL + encodeURI(kw) + "&page=" + pageno
    console.log(url)

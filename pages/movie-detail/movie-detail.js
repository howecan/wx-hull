...
Page({
  ...
  data: {
      movies:[],
      poster: app.globalData.download + "/images/wx-banner.jpg",
      article:'',
      imglist:'',
      jumpurl:''
  ...
  onLoad: function (options) {
     var that = this
     var id = options.id
     var url = app.globalData.ajaxBase + app.globalData.subject + id
     console.log(url)
     wx.showToast({
       title:'加载中',
       icon:'loading',
     ...
     wx.request({
       ...
       success:res => {
         ...
         this.setData({ id, title, writer, typename, typeid, pubdate, jumpurl, movies })
         this.setData({
           article: WxParse.wxParse('article', 'html', body, that, 0)
         })
         if(imgs != ''){
            this.setData({
              imglist: WxParse.wxParse('imglist', 'html', imgs, that, 0)
            })
         }
       },
